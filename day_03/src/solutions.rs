...
pub fn trees_in_path_with_default_step(input: &[&str]) -> u32 {
    trees_in_path(input, &STEP) as u32
}

pub fn multiplication_of_tress_in_path_with_different_steps(input: &[&str]) -> u64 {
    STEPS
        .iter()
        .fold(1, |acc: u64, item| acc * trees_in_path(input, item))
}

fn trees_in_path(input: &[&str], step: &(usize, usize)) -> u64 {
